...
//   </e>
	LPC_TIM0->MCR = 3;
																				//MULTIPLE THRESHOLDS
	/*
	//comment the two assignations above
	LPC_TIM0->MR0 = TimerInterval;
	LPC_TIM0->MR1 = TimerInterval*2;
	LPC_TIM0->MR2 = TimerInterval*4;
	LPC_TIM0->MR3 = TimerInterval*8;
	LPC_TIM0->MCR |= 
	 (1u <<0) //only interrupt wihtout reset (it has to continue) on the first match
	|(1u <<3)
	|(1u <<6)
	|(3u <<9); //interrupt and reset on the third match
	NVIC_SetPriority(TIMER0_IRQn, 4);


//generate interrupt without reset for each threshold except the last one
	*/
	//
// </h>
